...
class PolicyEstimator:
    ...
    def __init__(self, args, learning_rate=0.01, scope="policy_estimator"):
        ...
        with tf.variable_scope(scope,reuse=tf.AUTO_REUSE ):
            self.args = args
            self.input_size = args.state_space#30
            self.state_space = args.state_space#30
            self.hidden_size = args.hidden_size#100
            self.num_layers = args.num_layers#2
            self.actions_num = args.actions_num#10
            ...
            self.target = tf.placeholder(dtype=tf.float32, name="target")# reward-baseline-reward
            ...
            with tf.variable_scope("LSTM"):
                for time_step in range(self.actions_num):#4
                    ...
                    self.outputs.append(inputs)# the dimension is 4*30.
            for time_step in range(self.actions_num):
                if time_step == 0:
                    picked_action_prob = self.outputs[time_step][0,self.actions[time_step]]# picking out the probablity of the selected action.
                else:
            ...
            self.train_op = self.optimizer.minimize(
                self.loss, global_step=tf.train.get_global_step())#tf.train.get_global_step

    def predict(self, state, sess=None):
        sess = sess if sess else tf.get_default_session()
        return sess.run(self.outputs, {self.state: state})

    def update(self, state, target, actions, sess=None):#update(self.state, advantage, self.actions, self.sess)
...
class Controller:
    def __init__(self, args, scope="Controller"):
        self.args = args
        self.state = np.random.random(size=(1, args.state_space))#30
    ...
    def train_controller(self, reward):
        baseline_value = self.value_estimator.predict(self.state, self.sess)#always using the same input.
