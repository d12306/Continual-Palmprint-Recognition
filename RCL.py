...
class RCL:
    def __init__(self,args):
        ...
        print(np.asarray(self.task_list[0][1]).shape)
    ...
    def train(self):
        ...
        for task_id in range(0,self.num_tasks):
            self.best_params[task_id] = [0,0,0,0,0,0]
            if task_id == 0:
                with tf.Graph().as_default() as g:
                    with tf.name_scope("before"):
                        inputs = tf.placeholder(shape=(None, 128,128,3), dtype=tf.float32)
                        #[filter_height, filter_width, in_channels, out_channels]
                        filter1 = tf.Variable(tf.truncated_normal(shape=([3,3,3,16]), stddev=0.01))
                        
                        filter2 = tf.Variable(tf.truncated_normal(shape=([3,3,16,32]), stddev = 0.01))
                        
                        y = tf.placeholder(shape=(None, 50), dtype=tf.float32)
                        w1 = tf.Variable(tf.truncated_normal(shape=(2048,512), stddev=0.01))
                        b1 = tf.Variable(tf.constant(0.1, shape=(512,)))
                        w2 = tf.Variable(tf.truncated_normal(shape=(512,128), stddev=0.01))
                        b2 = tf.Variable(tf.constant(0.1, shape=(128,)))
                        w3 = tf.Variable(tf.truncated_normal(shape=(128,50), stddev=0.01))
                        b3 = tf.Variable(tf.constant(0.1, shape=(50,)))


                        output1 = conv2d(inputs, filter1)
                        output1 = tf.nn.relu(output1)
                        output1 = max_pool_2x2(output1)
                        output1 = max_pool_2x2(output1)

                        output2 = conv2d(output1, filter2)
                        output2 = tf.nn.relu(output2)
                        output2 = max_pool_2x2(output2)
                        output2 = max_pool_2x2(output2)
                        ...
                        output4 = tf.reshape(output2, [-1, 2048])
                        ...
                        loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=y, logits=output6)) + \
                        ...
                        accuracy = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(y,axis=1),tf.argmax(output6,axis=1)),tf.float32))
                        ...
                        for epoch in range(self.epochs):
                            ...
                            for _ in range(l//self.batch_size+1):
                                ...
                                sess.run(train_step,feed_dict={inputs:batch_xs, y:batch_ys})#training the classifier

                        accuracy_test = sess.run(accuracy, feed_dict={inputs:self.task_list[task_id][4], y:self.task_list[task_id][5]})# test the accuracy in the 
                        #testing set
                        #in the test set.
                        logging.info("test accuracy: "+str(accuracy_test) )
                        self.vars = sess.run([filter1, filter2, w1,b1,w2,b2,w3,b3])
...
if __name__ == "__main__":
    ...
    # data parameters
    parser.add_argument('--data_path', default='./data.pkl',
                        help='path where data is located')
    parser.add_argument('--state_space', type=int, default=30, help="the state space for search") 
    #parser.add_argument('--state_space_cnn', type = int, default = 10, help =' the state sapce for search the cnn layers.')
    parser.add_argument('--actions_num', type=int, default=4, help="how many actions to dscide")
    ...
    for index,value in params.items():
        logging.info([_.shape for _ in value[1]], file=f)
        accuracy.append(value[0])
    logging.info(accuracy,file=f)
    f.close()
    logging.info(fname)
    name = fname + '.pkl'
    f = open(name, 'wb')
